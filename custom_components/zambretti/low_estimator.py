...
def _wind_delta_knots(history: Any) -> float | None:
...
def _translate_wind_outlook(outlook: str | None) -> str | None:
    """Translate wind outlook sentence to Italian for user-facing summaries."""
    if not outlook:
        return None
    return (
        outlook.replace(
        ...
        .replace(
            ...
            "Segnale del vento ambiguo, monitorare con attenzione.",
        )
    )
...
def _derive_weather_trend(distance_class: str, wind_trend: str) -> str:
    ...
    if "Decreased" in wind_trend and distance_class in (
        "Far",
        "Distant",
        "Approaching",
    ):
...
def _derive_wind_rotation_likely(
    distance_class: str, wind_dir_delta_deg: float | None
) -> str:
    """
...
def _derive_frontal_zone(
    distance_class: str, pressure_slope_hpa_per_hr: float | None, wind_trend: str
) -> bool:
    """
    5) Front proximity heuristic.
    ...
    if pressure_slope_hpa_per_hr > -0.5:
        return False
    return "Increasing" in wind_trend
...
def _derive_anchoring_risk(distance_class: str, wind_trend: str) -> str:
    ...
    if distance_class == "Approaching":
        return (
            "Caution"
            if ("Increasing" in wind_trend or "Stable" in wind_trend)
            else "Safe"
        )
    return "Safe"
...
def estimate_low_properties(
    *,
    ...
    low_relative_position, low_movement, impact_window_status = (
        _derive_relative_position_and_movement(
    ...
    if wspd is None:
        ...
        trend_txt = trend_it.get(wind_trend, wind_trend.lower())
        if delta_kn is None:
            wind_clause += f" ({trend_txt})."
        else:
            wind_clause += f", {trend_txt} (Δ {'+' if delta_kn >= 0 else ''}{delta_kn:.1f} kn)."
        parts.append(wind_clause)
        outlook_it = _translate_wind_outlook(wind_outlook)
        if outlook_it:
            parts.append(outlook_it)

    gust_txt = gust_it.get(gust_likelihood, str(gust_likelihood).lower())
    if (
        ...
        parts.append(
            "Raffiche: "
            f"probabilita {gust_txt}, "
            f"attese ~{expected_gust_kn:.0f} kn (x{gust_factor:.1f} della media)."
        )
    else:
        parts.append(f"Raffiche: probabilita {gust_txt}.")
